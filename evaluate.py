...
from diffusion.ddpm import DenoiseDiffusion, gather
from trainers.trainer import Trainer
from sklearn.neighbors import KernelDensity

def class_conditioned_kernel_density_likelihood(trainer: Trainer, class_idx, dataset, n_samples=5000):
    samples = trainer.sample(n_samples, class_idx).cpu().numpy()
    kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(samples)
    return np.sum(kde.score_samples(dataset.samples.numpy())) / n_samples


def kernel_density_likelihood(trainer: Trainer, dataset, n_samples=5000):
    samples = trainer.sample(n_samples).cpu().numpy()
    kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(samples)
    return np.sum(kde.score_samples(dataset.samples.numpy())) / n_samples
